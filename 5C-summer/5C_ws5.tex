...
\begin{document}
\Large \noindent \textbf{Physics 5C Discussion (Worksheet 5)}\\
\normalsize
...
We have studied the electric field in a parallel-plate capacitor in worksheet 3. 
\begin{center}
\includegraphics[scale=0.85]{wiredCapacitor}
\end{center}
...
|\vec{E}| = Q/(\epsilon_0 A)\,,
$
with $A$ being the area of each of the plates and $Q$ being the amount of positive charges stored at the top plate. There is an equal amount of negative charges $-Q$ stored at the bottom plate. The electric field and potential that we study in worksheet 4 problem 2 are in fact created by a capacitor. From there, you see that the top and bottom plates have different electric potential, that we call a potential difference $\Delta V$, between the two plates.\\


\noindent \textbf{Part (a)} For a parallel-plate capacitor separated by a distance $d$, show that \footnote{Hint: Review what we did in worksheet 4 problem 2.}
...
\begin{align*}
U_c = \frac{1}{2}C(\Delta V)^2\,,\qquad \text{measured in Joules (J)}.
\end{align*}
\noindent \textbf{Part (b)} Using part (a), for the same parallel-plate capacitor, show that \footnote{Now you see that, even though we have defined $C$ in terms of $\Delta V$ and $Q$, the capacitance in fact only depends on the geometry (area $A$, separation distance $d$, and constant $\epsilon_0$) of the capacitor.}
\begin{align*}
...
\end{align*}
where $\rho$ is the resistivity (a natural property) of the conductor, $L$ is the length of the conductor, and $A$ is the cross-sectional area of the conductor through which the electrons are flowing.
\begin{center}
\includegraphics[scale=0.85]{wireLoop}
\end{center}
\noindent\textbf{Part (c)} Imagine there is a wire loop (two ends of a wire ``glued" together) with radius $1\,$meter, and Jinyan is able to drive a single electron circulating clockwise in this loop, with one second per revolution. Find the current $I$ in this wire. \footnote{In this picture, no battery is required. There is some magic that can make this electron move at a constant speed while circulating around the loop. You will learn about that magic later in the quarter.}\footnote{One electron has charge $-1.6 \times 10^{-19}$\,C. How would the negative sign in $Q$ (electron is negatively charged) affect the definition of a current?}\\
...
\noindent\textbf{Part (a)} From worksheet 4, for a uniform electric field, we have learned that the relation between electric field and electric potential is
\begin{align*}
...
\end{align*}
Here one electron goes around the loop once every second. That means if I pick any point on the loop, one electron passes by the point every one second. Therefore the amount of charge passing through the wire in $1$ second is just the charge of one electron,
\begin{align*}
...
\end{align*}
This is the amount of current flowing in the clockwise direction in the loop, which is equivalent to having the same amount of positive charges 
\begin{align*}
...
\begin{center}
\includegraphics[scale=0.15]{baiyunCat}\\
\hfill\break
A kitty at Baiyun Mountain in Guangzhou, China. Pictured in December 2025. 
\end{center}
