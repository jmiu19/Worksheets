...
\begin{document}
\Large \noindent \textbf{Physics 5C Discussion (Worksheet 3)}\\
\normalsize
...
\newpage
\noindent\textbf{Problem 2}: Now we revisit the problem on worksheet 2 again. 
\begin{center}
...
\noindent\textbf{Part (c)} If the $1\,$kg charge $q_3$ is initially at the point $\vec{x} = (5,0)$\,m, with an initial velocity $\vec{v} = (-1,0)$\,m/s, will it be able to pass through the point where it experiences zero net force? \footnote{Hint: From worksheet 1, $q_3$ experiences zero net force at $x = 4.14$\,m. Apply energy conservation to see whether $q_3$ has enough initial kinetic energy to overcome the potential energy difference between the starting location at $x=5$\,m and the target location at $x = 4.14$\,m.}
...
\end{align*}
That means, the electron should better be able to exit the region within $\sqrt{2h Am\epsilon_0/(|q|Q)}$ seconds, so that it does not hit the top plate. To exit the region, we want $\Delta x = L$\,m because the plates are $L$ meters long, so 
\begin{align*}
...
\end{center}
I have also labeled on the graph where $q_3$ starts its motion. Since it is initially moving to the left, it is initially ``climbing up a hill" in the middle. Depending on whether it can get to the top of the hill from the right, it will either roll to the left sink or roll to the right sink eventually. The top of the hill is in fact located at $x = 4.14$ which we found in worksheet 2 discussion. Why? On the left of $x=4.14$, $q_3$ wants to accelerate to the left as the attraction due to the left charge $q_1$ is too strong; On the right of $x= 4.14$, $q_3$ wants to accelerate to the right; Same thing applies to the ``hill" picture. To see whether $q_3$ can ``get to the top of the hill," we use conservation of energy, 
\begin{align*}
...
\begin{align*}
\text{KE}_i = \frac{1}{2}mv_{i,x}^2 = \frac{1}{2}\cdot (1\,\text{kg})\cdot (-1\,\text{m/s})^2 =0.5 \,\text{J}\,,
\end{align*}
