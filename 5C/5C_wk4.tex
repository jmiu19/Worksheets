...
\end{center}
\noindent Notice at all points where the blue lines (electric field lines) intersect with the read lines (equipotential lines), the two are perpendicular to each other. The blue lines on the right is denser than the blue lines on the left, and this is because the charge on the right is ``larger" than the charge on the left. The equipotential lines form a ``contour map," like the top view of two mountains, with the one on the right slightly larger than the one on the left.\\
...
In the argument above, we see that the constant term ``$+10$" in the original $V(x)$ does not matter, then the electric fields at $\vec{x}_1$ and $\vec{x}_2$, even if we change to $V(x)= x+ 233333$, stay the same as before,
\begin{align*}
\vec{E} = (-1,\,0)\,\text{N/m}\,.
\end{align*}
