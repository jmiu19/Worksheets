...
\noindent\textbf{Part (d)} At long time, since the current through $R_2$ and $R_3$ is zero, the voltage drop across each resistor is also zero because $V=IR$. Therefore the entire battery emf must appear across the capacitor,
\begin{align*}
V_C(t\to \infty)=\varepsilon\,.
\end{align*}
The reason that the charge capacitor will consume all supplied voltage from the battery is the following: Charging the capacitor means pumping charges to the capacitor, but storing more charges on the capacitor requires having a larger electric potential difference across the capacitor ($Q=CV$). The battery can supply an electric potential difference of $\epsilon$, thus it is able to pump charges to the capacitor until the potential difference across the capacitor reaches $\epsilon$.\\


A better way to understand this picture (when capacitor is fully charged) is that, the battery supplies a battery emf $\epsilon$ (this is a voltage gain if we go across the battery), and the capacitor will consume all of this voltage $\epsilon$ when it is fully charged (so there will be a voltage drop when we go across the capacitor). Therefore, there is no voltage left for the resistors (after the charged capacitor) to consume, and thus $I=V/R = 0$ for the resistors. \\ 



\noindent\textbf{Part (e)} When the capacitor is uncharged, the amount of charged stored on the capacitor is zero, so we have $Q=0$. The stored energy in a capacitor is
\begin{align*}
...
\end{align*}
At the initial time, the capacitor is uncharged, thus the voltage drop across it must be zero (since $C=Q/V$ for capacitance, then $V=Q/C$, so for an uncharged capacitor $Q=0$, we have $V = 0$). If you have this argument in your understanding and have it written down in your solution, you can also use
\begin{align*}
...
In another story, consider the case where the capacitor is disconnected from the circuit after it is being fully charged. If I then insert a dielectric material (assume $\kappa>1$) into the capacitor, again, the capacitance $C$ increases, but the voltage $V$ across the capacitor will decrease. This is because, the charge $Q$ stored in the capacitor is unchanged when the capacitor is disconnected from the circuit, then $V=Q/C$ decreases due to the increase in capacitance. Comparing the two stories here: One has the capacitor being connected to the battery at all time, so the battery can pump more charges to the capacitor to maintain the voltage across the capacitor. The other one has the capacitor being disconnected from the circuit, the amount of charges on the capacitor is maintained, and voltage across the capacitor will change correspondingly. \\
...
\noindent\textbf{Part (j)} Talking about the terminal voltage, we are looking at the voltage across the ``battery with its internal resistance." That is, we can consider a box that we call battery$+r$, and inside this box, we have the battery (with emf $\epsilon$) in series with a little resistor with resistance $r$. Then the terminal voltage is measuring the voltage across the two points right outside of the box. In this problem, outside of the box, the only thing that is in the circuit is the parallel resistor pair. Therefore, the terminal voltage of the battery is also the voltage across the parallel resistor pair (as they are the only thing connected to the battery+$r$ box),
\begin{align*}
