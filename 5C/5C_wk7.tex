...
\newpage
\noindent \textbf{Problem 2}: Current produces a magnetic field. If we wind the wire into a solenoid, the magnetic field strength produced by the current in the wire is uniform inside the solenoid, with direction determined by the right-hand rule and magnitude given by  
\begin{align*}
|\vec{B}| = \mu_0 N I/L \,,
\end{align*}
where $\mu_0$ is a constant, $N$ is the number of wire ``loops" in a total solenoid length of $L$, $I$ is the current in the wire. \\

\noindent (a) Explain why the magnetic field inside a long (but has a finite length) solenoid is stronger and more uniform than the field outside.\\

\noindent (b) Jinyan needs a $1\,$T magnetic field for his science innovation. He wants to build it using a gigantic solenoid. He has a wire that is in total $100$\,m long, and he plans to wind the wire into a solenoid that has radius $r = 1\,$m and a length of $10\,$m. How much current does he need in the wire to produce the amount of magnetic field that he wants.\\
...
\noindent (a) In what direction are the electrons in the wires moving?\\

\noindent (b) Why do the two wires repel each other?\\

\noindent (c) If the current in the wire is $50$\,$\mu$A, the distance between the two wires is $1\,\text{m}$, and an electron in the top wire is moving $1\,\text{nm/s}$, what is the repelling force experienced by the electron? Include both direction and magnitude in your answer.\\
...
\noindent\textbf{Part (a)} A solenoid is just many circular current loops stacked next to one another. Each loop produces a magnetic field. Inside the solenoid, the magnetic fields from neighboring loops point mostly in the same direction, so they add together. Outside the solenoid, the fields from neighboring loops tend to point in different directions and partially cancel. This is why the magnetic field inside a long solenoid is much stronger than the field outside. The field inside is also more uniform because, far from the two ends towards the middle of the solenoid, each turn of the solenoid contributes in almost the same way. Near the ends the symmetry is less perfect, so the field becomes less uniform there.\\
...
\noindent\textbf{Part (c)} Now the problem gives us all the numbers that we need. The current in the wire is
\begin{align*}
I = 50\,\mu\text{A} = 50\times 10^{-6}\,\text{A} = 5\times 10^{-5}\,\text{A}\,,
\end{align*}
the wire separation is
\begin{align*}
d = 1\,\text{m}\,,
\end{align*}
and the electron speed is
\begin{align*}
v = 1\,\text{nm/s} = 10^{-9}\,\text{m/s}\,.
\end{align*}
